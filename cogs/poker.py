...
FIXES:
  - Timeout de mesa inativa: 10 min sem ação encerra e devolve stacks
  - Mesas não crasham mais se o bot reiniciar (botões ignoram mesa inexistente)
  - PKActionsView usa timeout=600 em vez de None para evitar views eternas
"""
...
MESA_TIMEOUT = 600   # 10 min sem ação → encerra e devolve fichas
...
async def _cancelar_timeout(game: PKGame):
    if game._timeout_task and not game._timeout_task.done():
        game._timeout_task.cancel()
        game._timeout_task = None


async def _agendar_timeout(game: PKGame, bot):
    """Encerra mesa após MESA_TIMEOUT segundos sem ação. Devolve stacks."""
    await _cancelar_timeout(game)

    async def _expirar():
        await asyncio.sleep(MESA_TIMEOUT)
        if mesas.get(game.canal) is not game: return
        # Devolver fichas de volta
        for p in game.jogadores:
            if p.stack > 0:
                await add_saldo(bot, p.user, p.stack)
        mesas.pop(game.canal, None)
        if game.msg:
            try:
                await game.msg.edit(
                    content="⏰ **Mesa encerrada por inatividade.** Stacks devolvidos.",
                    attachments=[]
                )
...
async def _render_update(game: PKGame, current_name=""):
    ...
    buf = render_poker(game.community, players, game.pot, current_name, mobile=True)
    try:
...
async def _avancar_fase(game: PKGame, bot, interaction: discord.Interaction):
    ...
    if a:
        ...
        await _agendar_timeout(game, bot)
...
class PKActionsView(discord.ui.View):
    ...
    async def on_timeout(self):
        # View expirou — a task de timeout do game vai limpar tudo
        pass
...
class PKNextView(discord.ui.View):
    ...
    async def nova(self, interaction, btn):
        ...
        g.jogadores = [p for p in g.jogadores if p.stack > 0]
        if len(g.jogadores) < 2:
            mesas.pop(self.canal, None)
            return await interaction.followup.send(
                "Mesa encerrada — jogadores sem fichas.", ephemeral=True)
...
async def _iniciar_mao(g: PKGame, bot, interaction: discord.Interaction):
    ...
    n = len(g.jogadores)
    for p in g.jogadores: p.dealer = False
    ...
    buf = render_poker([], players, g.pot, g.jogadores[g.cur].user.display_name, mobile=True)
