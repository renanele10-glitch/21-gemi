...
_BJ_SLOTS  = [(201,470,-14),(687,460,0),(1128,460,14)]
_BJ_DEALER = (687, 207)
...
def render_blackjack(slots, dealer_cards, dealer_val, reveal_dealer=False, result_map=None):
    ...
    # Jogadores — distribui slots baseado na quantidade
    n_slots = min(len(slots), 3)
    if n_slots == 1:
        slot_indices = [1]           # centro
    elif n_slots == 2:
        slot_indices = [0, 2]        # esq + dir
    else:
        slot_indices = [0, 1, 2]     # todos

    for i, slot in enumerate(slots[:3]):
        si = slot_indices[i]
        cx, cy, angle = _BJ_SLOTS[si]
        cw, ch = _BJ_SLOT_CW[si], _BJ_SLOT_CH[si]
