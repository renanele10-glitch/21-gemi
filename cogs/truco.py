...
Mãos enviadas por DM. Ações via botões.
"""
...
NAIPES = ["♦","♠","♥","♣"]   # ordem das manilhas
PREMIO = 100   # fichas por vencer a mão
...
def _parse(c): return c   # já é (rank, suit)
...
class TRGame:
    ...
    mesa: list      = field(default_factory=list)   # [(rank,suit,nome)]
    cur_idx: int    = 0
    truco_val: int  = 2
...
async def _render(g: TRGame):
    ...
    buf = render_truco(nos, eles, g.pontos["nos"], g.pontos["eles"],
                       g.vira, g.mesa, g.rodada, g.max_pts)
    try: await g.msg.edit(attachments=[discord.File(buf,"truco.png")])
    except Exception: pass


async def _enviar_maos(g: TRGame):
    # Sem DM — cada jogador vê sua mão clicando em "Ver Minha Mão" (ephemeral)
    pass
...
async def _fim_rodada(g: TRGame, bot, interaction: discord.Interaction):
    ...
    # Verificar fim de mão (2 rodadas ganhas)
    if g.rodada_pts["nos"] >= 2 or g.rodada_pts["eles"] >= 2:
        await _fim_mao(g, bot, interaction); return

    if g.rodada >= 3:
        # Empate na terceira = quem ganhou a primeira leva
        await _fim_mao(g, bot, interaction); return
...
class TRAcoesView(discord.ui.View):
    ...
    async def jogar(self, interaction, btn):
        ...
        if not j: return await interaction.response.send_message("Não é sua vez.", ephemeral=True)
...
async def _iniciar_mao_tr(g: TRGame, bot, interaction):
    ...
    buf  = render_truco(nos, eles, 0, 0, g.vira, [], 1, g.max_pts)
    msg  = await interaction.channel.send(
        file=discord.File(buf,"truco.png"), view=TRAcoesView(g.canal, bot))
    g.msg = msg
    await _enviar_maos(g)
    await interaction.followup.send(
