...
Comandos:
  /admin_reset @usuario [valor]  — Reseta fichas de um usuário (padrão: saldo inicial)
  /admin_dar @usuario valor      — Adiciona fichas (pode ser negativo para remover)
  /historico [@usuario]          — Mostra histórico de W/L e saldo
  /admin_resetar_todos           — Reseta TODOS os saldos (confirmação obrigatória)

Permissão exigida: Manage Guild (administrador do servidor).
"""
import discord
...
def _admin():
    """Check de permissão: Manage Guild."""
    async def predicate(interaction: discord.Interaction) -> bool:
        if not interaction.user.guild_permissions.manage_guild:
            await interaction.response.send_message(
                "❌ Sem permissão (necessário Manage Guild).", ephemeral=True
            )
            return False
        return True
...
class ConfirmarResetView(discord.ui.View):
    """View de confirmação para resetar todos os saldos."""
...
class Admin(commands.Cog):
    ...
    @app_commands.describe(
        usuario="Usuário a resetar",
        valor=f"Novo saldo (padrão: {SALDO_INICIAL})"
    )
    @_admin()
    async def cmd_reset(
        self,
        ...
        await interaction.response.send_message(
            f"✅ **{usuario.display_name}**: {saldo_antes:,} → **{valor:,}** fichas.",
            ephemeral=True
        )
    ...
    @app_commands.describe(
        ...
        valor="Fichas a adicionar (negativo para remover)"
    )
    @_admin()
    async def cmd_dar(
        self,
        ...
        await interaction.response.send_message(
            f"✅ **{usuario.display_name}**: {saldo_antes:,} → **{saldo_depois:,}** fichas "
            f"({sinal}{valor:,}).",
            ephemeral=True
        )

    # ── /historico ────────────────────────────────────────────────────────────
    @app_commands.command(name="historico", description="Histórico de vitórias/derrotas e saldo")
    ...
    async def cmd_resetar_todos(self, interaction: discord.Interaction):
        await interaction.response.send_message(
            ...
            "Tem certeza?",
            view=ConfirmarResetView(self.bot),
            ephemeral=True
        )
